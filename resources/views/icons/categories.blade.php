...
<button class="btn" type="button" id="dropdownMenu2" data-toggle="dropdown" aria-expanded="false" data-reference="#accordionMain"> 
  ...
  <ul class="dropdown-menu no-min-width" aria-labelledby="dropdownMenu2">
    ...
    @can('edit-categories')
    <li>
        <button class="dropdown-item" type="button" data-bs-name="{{ $category->name }}" data-toggle="modal" data-target="#editCategories" data-bs-id="{{ $category->id }}" data-bs-parent="{{ $category->category_id }}" data-hint="true" data-placement="right" title="Редактировать">
